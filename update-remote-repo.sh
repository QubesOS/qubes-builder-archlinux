...
# Include XEN 4.6.*
sudo chroot chroot-archlinux su user -c 'cd /tmp/qubes-packages-mirror-repo; for pkg in `ls -v pkgs/qubes-vm-xen-4.6.*.pkg.tar.xz` ; do repo-add pkgs/qubes-r3.1.db.tar.gz "$pkg";done;'

echo "Signing all repositories"
for repo in "qubes-r3.1" "qubes-r3.2"; do
